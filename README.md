...
- **[app-store-reviews-responder](apple/app-store-reviews-responder.skill)** — Monitors App Store customer reviews and ratings. Fetches recent reviews, summarizes themes, and drafts professional, on-guideline developer responses.
- **[appstore-media](apple/appstore-media.skill)** — End-to-end pipeline for producing App Store-compliant screenshots and App Preview videos for iOS and macOS apps, driven by a scripted, repeatable XCUITest demo flow.
...
- **[apple-bug-flow-review](apple/apple-bug-flow-review/SKILL.md)** — Functional QA audit for iOS/macOS apps before shipping: finds real logic/correctness bugs (force-unwraps, data races, retain cycles, `@StateObject` mistakes, edge cases, data loss), code-robustness issues (resource leaks, lifecycle, error handling, networking, debug leftovers), and broken user flows (dead-end navigation, unhandled empty/error/offline/permission-denied states, lost unsaved data, interruptions). Scans statically, runs the analyzer, and builds & runs on the simulator with sanitizers + XCUITest smoke flows, then produces a severity-ranked report with repro steps and fixes.
...
- **[aso-keywords](apple/aso-keywords.skill)** — App Store Optimization (ASO) for discoverability. Audits and optimizes the app name, subtitle, and 100-character keyword fields per locale to improve search ranking.
...
- **[localization-i18n](apple/localization-i18n.skill)** — Audits and manages in-app localization (Localizable.strings / String Catalogs) for Xcode projects. Finds missing keys, untranslated strings, and hardcoded UI strings, and helps handle RTL and pluralization.
